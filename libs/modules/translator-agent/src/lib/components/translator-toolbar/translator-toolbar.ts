 ...
 * queue, performance report, feedback) available from every translator-agent
 * page. Wraps the shared `@nfinyx/document-agent` drawers and owns their
 * fetching — pages just drop `<lib-translator-toolbar>` into their header.
 */
@Component({
    selector: 'lib-translator-toolbar',
    standalone: true,
    imports: [
        CommonModule,
        TranslateModule,
    ...
    templateUrl: './translator-toolbar.html',
})
export class TranslatorToolbarComponent implements OnInit {
    ...
    private toReviewQueueItem(doc: DocumentSummary): ReviewQueueItem {
        return {
            ...
            title: doc.filename,
            metaLine: [
                doc.document_type || this.translate.instant('translatorAgent.common.unclassified'),
                doc.issued_by_name || this.translate.instant('translatorAgent.common.unknownIssuer'),
            ],
            badges: [
                ...(doc.confidence_score !== null
                    ? [{ label: `${doc.confidence_score}%`, severity: this.confidenceSeverity(doc.confidence_score) as 'success' | 'warn' | 'danger' }]
                    : []),
                { label: this.translate.instant(`translatorAgent.risk.${doc.risk_level}`) || doc.risk_level, severity: this.riskSeverity(doc.risk_level) },
            ],
        };
    }
    ...
    openDocumentFromQueue(id: string): void {
        this.drawer = 'none';
        this.router.navigateByUrl(`/translator-agent/documents/${id}`);
    }
    async onReviewDecide(payload: ReviewDecisionPayload): Promise<void> {
        try {
            ...
            this.toastr.success(
                this.translate.instant(payload.decision === 'approve' ? 'translatorAgent.toast.reviewApproved' : 'translatorAgent.toast.reviewRejected'),
            );
            await this.loadReviewQueue();
        } catch (err) {
    ...
    async onSubmitFeedback(payload: FeedbackSubmitPayload): Promise<void> {
        try {
            ...
            this.drawer = 'none';
            this.toastr.success(this.translate.instant('translatorAgent.toast.feedbackSubmitted'));
        } catch (err) {
            this.toastr.error(this.errorMessage(err, 'translatorAgent.toast.feedbackFailed'));
        }
    }
