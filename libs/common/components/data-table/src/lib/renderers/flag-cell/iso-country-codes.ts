/**
 * ISO 3166-1 alpha-2 / alpha-3 country code lookup, used to resolve a country flag image or
 * display name from a raw code or full country name (e.g. {@link FlagCellComponent}).
 */
...
export const ISO_COUNTRIES: IsoCountry[] = [
    ...
    { iso2: 'CI', iso3: 'CIV', name: "Cote d'Ivoire" },
...
const byIso2 = new Map<string, IsoCountry>(ISO_COUNTRIES.map(c => [c.iso2, c]));
const byIso3 = new Map<string, IsoCountry>(ISO_COUNTRIES.map(c => [c.iso3, c]));
const byName = new Map<string, IsoCountry>(ISO_COUNTRIES.map(c => [c.name.toUpperCase(), c]));

/**
 * Resolves an ISO 3166-1 alpha-2/alpha-3 code or a country name (all case-insensitive) to its
 * country record. Codes are matched by length (2 => alpha-2, 3 => alpha-3); anything else, or a
 * code that doesn't match, falls back to an exact name match.
 */
export function resolveIsoCountry(code: string | null | undefined): IsoCountry | undefined {
    if (!code) return undefined;
    const normalized = code.trim().toUpperCase();
    if (normalized.length === 2) return byIso2.get(normalized) ?? byName.get(normalized);
    if (normalized.length === 3) return byIso3.get(normalized) ?? byName.get(normalized);
